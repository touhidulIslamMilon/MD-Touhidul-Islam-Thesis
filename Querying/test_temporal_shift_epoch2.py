...
prompts= [
"Right now, it feels like <mask> outside.",
...
"At this moment, the season is <mask>."
]
...
def main():
    ...
    results_single.to_csv(f"ten/withoutsim/Single/TemporalShiftepoch2single_query_epoch{epoch_num}.csv")
    
    results_average = get_average(result_list, 'average')
    results_average.to_csv(f"ten/withoutsim/Mean/TemporalShiftepoch2Multi_mean_query_epoch{epoch_num}.csv")
    
    results_median = get_average(result_list, 'median')
    results_median.to_csv(f"ten/withoutsim/Median/TemporalShiftepoch2Multi_median_query_epoch{epoch_num}.csv")
