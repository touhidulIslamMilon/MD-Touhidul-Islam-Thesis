...
Description: Fine-tuning BERT model for Twitter data (2019 Q4)
"""
...
year_num = 2019  # Numeric year value
half = "_1"  # Half of the year identifier
name = "twhin-bert-base_retrain"  # Model name prefix
...
def whole_word_masking_data_collator(features):
    ...
    for feature in features:
        ...
        # Randomly mask words based on probability
        mask = np.random.binomial(1, wwm_probability, (len(mapping),))
        input_ids = feature["input_ids"]
        labels = feature["labels"]
        new_labels = [-100] * len(labels)  # Initialize with ignored index
        
        # Apply masking and set corresponding labels
        for word_id in np.where(mask)[0]:
            word_id = word_id.item()
            for idx in mapping[word_id]:
                new_labels[idx] = labels[idx]  # Set label to original token
                input_ids[idx] = tokenizer.mask_token_id  # Replace with mask token
...
def get_eval_data(insert_random_mask, downsampled_dataset):
    ...
    """
    # Remove word_ids column as it's no longer needed
    downsampled_dataset = downsampled_dataset.remove_columns(["word_ids"])

    # Apply masking to the test dataset
    ...
    # Rename columns to standard format expected by the model
...
# Main training loop for multiple periods
for i in range(0, 5): # Change the number here to adjust training periods
    # Initialize model and optimizer for this iteration
    model = AutoModelForMaskedLM.from_pretrained(model_checkpoint)
    distilbert_num_parameters = model.num_parameters() / 1_000_000
    optimizer = AdamW(model.parameters(), lr=5e-5)
    
    # Calculate year and week for this iteration
    year = str(year_num+i)
    Week = year+"_"+ str(53)

    # Set model name and file path
    ...
    # Remove word_ids column as it's no longer needed for training
    downsampled_dataset = downsampled_dataset.remove_columns(["word_ids"])
    
    # Prepare test dataset with masked inputs
