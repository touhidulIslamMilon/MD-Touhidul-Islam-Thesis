...
Description: Fine-tuning BERT model for Twitter data (2019 Q3)
"""
...
from traning_function import training_function, training_function2
...
# Commented alternative initialization
# tokenizer = AutoTokenizer.from_pretrained('Twitter/twhin-bert-base')
# model = AutoModel.from_pretrained('Twitter/twhin-bert-base')

# Training Parameters
chunk_size = 128  # Size of text chunks for processing
percent_trian = 1  # Percentage of data to use for training
year = "2019"  # Year of data
quater = "3quater"  # Quarter identifier
starting_week = 22  # Starting week for training
ending_week = 40  # Ending week for training
...
def process_data_quaterly(starting_week,ending_week):
    """
    Process data across multiple weeks, combining them into a single dataset.
    
    Args:
        starting_week: First week to include in the dataset
        ending_week: Last week to include in the dataset
        
    Returns:
        lm_datasets: Processed dataset ready for language modeling
    """
    # Format the week number and create filename for the first week
    if starting_week < 10:
        Week = year+"_0"+ str(starting_week)
    else:
        Week = year+"_"+ str(starting_week)
    filename = "/pfs/work7/workspace/scratch/ma_mislam-mislam_twitterlm_thesis/Code/files/"+Week+"/"+Week+"_tweet_text"
    dataset= load_dataset('text', data_files = [filename])
    print("Filename: "+filename)
    
    # Iterate through remaining weeks and concatenate datasets
    for(i) in range(starting_week,ending_week):
        if i < 10:
            Week = year+"_0"+ str(i)
        else:
            Week = year+"_"+ str(i)
        filename = "/pfs/work7/workspace/scratch/ma_mislam-mislam_twitterlm_thesis/Code/files/"+Week+"/"+Week+"_tweet_text"
        print("Filename: "+filename)
        data = load_dataset('text', data_files = [filename])
        print("Dataset:")
        print(dataset)
        print("data:")
        print(data)
        dataset['train'] = concatenate_datasets([data['train'], dataset['train']])
...
def insert_random_mask(batch):
    ...
    Returns:
        Dictionary with masked versions of input features
    """
...
def get_eval_data(insert_random_mask, downsampled_dataset):
    ...
    Args:
        insert_random_mask: Function to apply masking
        downsampled_dataset: Dataset to prepare for evaluation
        
    Returns:
        eval_dataset: Processed evaluation dataset
    """
...
model_name = "Bert_twi_finetuned_mislam_"+quater

lm_datasets = process_data_quaterly(starting_week,ending_week)
...
result = training_function(model,optimizer,tokenizer, chunk_size, batch_size, wwm_probability, data_collator, model_name, downsampled_dataset, test_dataset,eval_dataset)
